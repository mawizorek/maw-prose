...
- ⬜ Event classification: a CU-side canonical-type dropdown was declined, so a **crosswalk** (raw name → `fkStandardEvent`) with an unmatched queue carries it. Learns instead of guessing. See [[STANDARD_EVENTS](@table-standard-events)].
