...
## ⬜ Also queued — device handoff

**Three fields and one script trigger, no relationships, no dependencies.** The open-log stamp (`LastOpenedAt` / `LastOpenedDevice` / `LastOpenedPlatform`) answers *"is this the newest copy"* on sight. Full spec, the four tiers, and the scripting implications of a later UI/DATA split: [[Device Handoff](@device-handoff)].
