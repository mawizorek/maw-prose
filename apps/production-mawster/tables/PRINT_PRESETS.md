...
A session carries `fkPreset`. Same rule as everywhere else today: **split by lifetime, not by topic.** The legacy file put these on `SETUP_EXPORT` as globals, which is why every reissue meant retyping the same settings.
