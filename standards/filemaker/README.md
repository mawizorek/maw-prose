...
`custom-functions/` holds the shared function families. A family is a small set of functions that solve one job, defined once here and **installed** into each solution — never re-derived per file. Two families exist:
